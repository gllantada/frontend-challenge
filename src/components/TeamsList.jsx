...
export function TeamsList() {
  ...
  function getScores(team) {
    let score = 0;
    team.games.forEach((elm) => (score += elm.score));
    return score;
  }
  ...
  function orderTeamByScoreHighestToLowest() {
    const orderTeams = teams
      .slice()
      .sort((a, b) => getScores(b) - getScores(a), true);
    setTeams(orderTeams);
  }
  ...
  function orderTeamByScoreLowestToHighest() {
    const orderTeams = teams
      .slice()
      .sort((a, b) => getScores(a) - getScores(b));
    setTeams(orderTeams);
  }

  // Filtering teams that with at least 3 players
  function teamsWithMoreThanThreePlayers() {
    const filterTeams = teams.filter((team) => team.players.length > 3);
